import Ajv from 'ajv';
import RootDirectory from 'app-root-path';
import _ from 'lodash';
import path from 'path';
import configMetaSchema from './config-definition.schema.json';
 ...
 */
const DEFAULT_CONFIG_ROOT = path.join(RootDirectory.path, 'config');
...
function loadConfig(configRoot: string): IConfigExportObject {
    let ret: IConfigExportObject = {_loadDbgConfig: loadConfig, errors};
 ...
 *
 * @throws **InternalConfigMetaValidationError** If the configuration definition is invalid.
 */
function loadConfigDefinition(configRoot: string): IConfigDefinition {
    ...
    const configMetaJSON = requireJSON(configMeta);
...
function loadConfigEnvironment(root: string, config: IConfigEnvironment): IConfigExportObject {
    const ret: IConfigExportObject = {_loadDbgConfig: loadConfig, errors};
...
function loadConfiguration(root: string, sources: string[], file: IConfiguration): any {
    ...
    for (const f of files) {
        try {
            ...
            if (!(error.name === 'FILE_NOT_FOUND_ERROR')) {
                // throw new Error(error.constructor.name);
 ...
 *
 * @param file The file path. Relative to project root.
 */
function requireJSON(file: string): any {
    const fullPath = path.resolve(RootDirectory.path, file);

    try {
        return require(fullPath);
    } catch (error) {
        throw new errors.FileNotFoundError(fullPath);
    }
}
