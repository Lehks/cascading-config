...
describe('core', () => {
    describe('config.ts', () => {
        ...
        it('should throw if the config is in an invalid schema', () => {
            ...
            expect(() => config._loadDbgConfig('test/config/cfg7'))
                .toThrow(new config.errors.ConfigValidationError('test', objValError));
        });

        it('should throw if the config metadata is in an invalid schema', () => {
            expect(() => config._loadDbgConfig('test/config/cfg8'))
                .toThrow(new config.errors.InternalConfigMetaValidationError(
        ...
        it('should throw if a config file can not be parsed', () => {
            /*expect(() => config._loadDbgConfig('test/config/cfg9'))
        ...
        it('should throw if a config environment has a nonexistent dependency', () => {
            expect(() => config._loadDbgConfig('test/config/cfg10'))
                .toThrow(new config.errors.ConfigExtensionError('nonexistent'));
        });

        it('should correctly load a config with dependencies', () => {
            const cfg = config._loadDbgConfig('test/config/cfg11');
        ...
        it('should work with a complex example', () => {
            const cfg = config._loadDbgConfig('test/config/cfg12');
        ...
        it('should correctly work if only the extension has sources', () => {
            const cfg = config._loadDbgConfig('test/config/cfg13');
            expect(cfg.test.test).toBe('test');
        });

        it('should correctly work if there are no configurations (empty array)', () => {
            expect(() => config._loadDbgConfig('test/config/cfg14')).not.toThrow();
        });

        it('should correctly work if there are no configurations (undefined)', () => {
            expect(() => config._loadDbgConfig('test/config/cfg15')).not.toThrow();
        });

        it('should correctly work if there are no sources', () => {
            expect(() => config._loadDbgConfig('test/config/cfg16')).not.toThrow();
        });
        ...
        describe('undefined NODE_ENV', () => {
            ...
            it('should load the correct configuration when no NODE_ENV is set', () => {
                const cfg = config._loadDbgConfig('test/config/cfg4');
                expect(cfg.test.test).toBe('test');
            });
        });
    });
